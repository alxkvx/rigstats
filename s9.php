...
function miner_details($type,$arr) {
	...
	$freq           = $json['stats'][0]['STATS'][1]['frequency'];
		...
		else if	(preg_match('/staiki NO/', $fw_type)) { $fw_type = 'Staiki'; }
	...
	else if (preg_match('/mining-dutch.nl/', $pool_url[$poolnum])) { $pool_url[$poolnum] = 'mining-dutch'; }
	else if (preg_match('/btc.viabtc.com/', $pool_url[$poolnum])) { $pool_url[$poolnum] = 'ViaBTC'; }
	...
	else if (preg_match('/staiki-pool.org/', $pool_url[$poolnum])) { $pool_url[$poolnum] = 'Staiki'; }
...
$html = '<link href="main.css" type="text/css" rel="stylesheet"/><head><title>A: S9</title></head>
...
foreach($s9 as $x) {
	...
	if ($poolname == 'ViaBTC') { $viabtchr+= $vars[2];$viabtcw++;}
	else if ($poolname == 'sha256.antpool.com')	{$slushhr += $vars[2];$slushw++;}
}

if ($avail>0) {$availclass = 'fyellow';} else {$availclass = '';}
if ($offline>0) {$offclass = 'fred';} else { $offclass = '';}

$html .= "<tr><td colspan=2><span class=\"box fiol\"><a href=\"add.php?type=s9\" target=\"_blank\">ADD</a></span></td></tr></table><br><span class=bold>Total: ". $s9num . " miners (<span class=fgreen>$online</span> / <span class=$availclass>$avail</span> / <span class=$offclass>$offline</span>)" . " | <span class=fblue>". number_format($totalavg/$online/1000,3)."</span> T (S9avg) | " . number_format($totalhashrate/1000,1) . " T | <span class=fblue>".number_format($totalavg/1000,1) ."</span> T(avg) | ".round($total_temp/$s9numt,2)."&deg; (avg) | <span class=forange>" .number_format($total_pwr/1000,2)."</span> kWt | Reject Rate: ". round($rejrateall/$s9numt,3) ."% || Load time: " . round(microtime(true) - $start, 3) . " sec (" . date('Y-m-d H:i:s') .") ViaBTC: ".number_format($viabtchr/1000,1)." T($viabtcw) | Ant: ".number_format($slushhr/1000,1)." T($slushw)<br><br>";
